...
var ccd3 = function () {
  ...
  ccd3.Chart = function (div_id, dataset) {
    this.div_id = div_id;
    ccd3.instances[div_id] = this; // ccd3.Parts holder
    ...
    this.dataset_manager = {}; // ccd3.Chart options
    ...
    this.dataset_to_csv_array = undefined; // ccd3.DatasetLoader
  ...
  ccd3.Chart.prototype.resize = function () {
    this.width = d3.select("#" + this.div_id)[0][0].clientWidth || this.width; //this.height = (d3.select("#"+this.div_id)[0][0].clientHeight || this.height); tmp
  ...
  ccd3.Chart.prototype.render = function (cond) {
    ...
    if (this.auto_resize) {
      ...
      d3.select(window).on("resize." + this.div_id, function () {
        that.resize();
        that.render({
          dont_recalc_dataset: true,
          dont_reset_domain: true
        });
      });
    }
    ...
    if (this.chart_pattern === "xy") {
      ...
      this.legend.arrange(this.yAxis.sizeof("label_width") + this.yLabel.sizeof("width"), this.title.sizeof("height"));
      this.inner_height = this.height - this.title.sizeof("height") - this.legend.sizeof("height") - this.xAxis.sizeof("label_height") - this.xLabel.sizeof("height");
      this.xAxis.arrange(this.yLabel.sizeof("width") + this.yAxis.sizeof("label_width"), this.height - this.xAxis.sizeof("label_height") - this.xLabel.sizeof("height"));
      this.yAxis.arrange(this.yLabel.sizeof("width") + this.yAxis.sizeof("label_width"), this.title.sizeof("height") + this.legend.sizeof("height"));
      this.series_container.render(this.inner_width, this.inner_height);
      this.series_container.arrange(this.yLabel.sizeof("width") + this.yAxis.sizeof("label_width"), this.title.sizeof("height") + this.legend.sizeof("height")); // must call after SeriesContainer setup

      this.event_layer = new ccd3.Parts.EventLayer(this, this.event_layer);
      this.event_layer.render(); // must call after EventLayer setup
      ...
      this.legend.arrange((this.width - legend_width) / 2, (this.inner_height - radius * 2) / 2 + this.title.sizeof("height"));
      ...
      this.aAxis.render();
      this.series_container.render(this.inner_width, this.inner_height);
      this.series_container.arrange(0, this.title.sizeof("height") + this.legend.sizeof("height"));
      this.rAxis.arrange(this.inner_width / 2, this.inner_height / 2 + this.title.sizeof("height") + this.legend.sizeof("height"));
      this.aAxis.arrange(this.inner_width / 2, this.inner_height / 2 + this.title.sizeof("height") + this.legend.sizeof("height"));
    } else if (this.chart_pattern === "r") {
      this.inner_width = this.width;
      this.inner_height = this.height - this.title.sizeof("height");
      this.series_container.render(this.inner_width, this.inner_height);
      this.series_container.arrange(0, this.title.sizeof("height"));
  ...
  ccd3.Chart.prototype.init = function (cond) {
    if (!this.color_palette) {
      ...
      this.color_palette = ["#3498db", "#e74c3c", "#2ecc71", "#f1c40f", "#e67e22", "#1abc9c", "#2980b9", "#c0392b", "#27ae60", "#f39c12", "#d35400", "#16a085"];
    }
    ...
    if (!cond.dont_recalc_dataset) {
      ...
      this.dataset = this.dataset_manager.setup(this.dataset); // must execute before execute this.init_parts()
    ...
    if (this.svg === undefined) {
      this.svg = d3.select("#" + this.div_id).append("svg").attr("id", "svg_" + this.div_id).attr("class", "ccd3").attr("overflow", "hidden") // for IE
      .attr("font-size", "10px").attr("font-family", "Lucida Grande,Hiragino Kaku Gothic ProN,Meiryo,sans-serif").attr("xmlns", "http://www.w3.org/2000/svg").attr("version", 1.1);
    }
    ...
    if (this.chart_pattern === "xy") {
      ...
      this.yLabel.arrange(this.yLabel.margin_left, this.height / 2); // calc_min_step return undefined if series has only one data.

      this.xAxis.scale_type = this.dataset_manager.detect_scale_type(function (d) {
        return d.x;
      });
      this.xAxis.min_step = this.dataset_manager.calc_min_step(function (d) {
        return d.x;
      }) || 2.0;
      this.yAxis.scale_type = this.dataset_manager.detect_scale_type(function (d) {
        return d.y;
      });
      this.yAxis.min_step = this.dataset_manager.calc_min_step(function (d) {
        return d.y;
      }) || 2.0;
      ...
      var z_scale_type = this.dataset_manager.detect_scale_type(function (d) {
        return d.z;
      });
  ...
  ccd3.Parts.prototype.init = function (chart, options) {
    ...
    this.chart = chart; // copy properties to this
  ...
  ccd3.Parts.SeriesContainer = function () {
  ...
  ccd3.Parts.SeriesContainer.prototype.render = function (width, height) {
    ...
    if (!this.svg) {
      this.svg = this.chart.svg.append("g").attr("class", "ccd3_series_container").attr("clip-path", "url(#" + this.chart.div_id + "_clip)"); // clip path

      this.clip_path = this.chart.svg.append("defs").append("clipPath").attr("id", this.chart.div_id + "_clip");
      this.clip_path.append("rect");
    }

    this.svg.attr("width", this.width).attr("height", this.height);
    this.clip_path.select("rect").attr("width", this.width).attr("height", this.height);
  };
  /* ------------------------------------------------------------------ */
  ...
  ccd3.Parts.Label.prototype.render = function () {
    if (this.svg === undefined) {
      ...
      this.svg.attr("class", this.class_name).append("text").attr("text-anchor", this.text_anchor);
    }

    this.svg.select("text").text(this.text).attr("font-size", this.font_size).attr("transform", "translate(0," + this.font_size + ") rotate(" + this.rotate + ")");
  ...
  ccd3.Parts.Label.Title = function () {
    ccd3.Parts.Label.apply(this, arguments);
  };
  ...
  ccd3.Parts.Label.xLabel = function () {
    ccd3.Parts.Label.apply(this, arguments);
  };
  ...
  ccd3.Parts.Label.yLabel = function () {
    ccd3.Parts.Label.apply(this, arguments);
  };
  ...
  ccd3.Parts.Legend.prototype.render = function (width) {
    ...
    var item = items.selectAll("g").data(data); // enter

    item.enter().append("g").attr("class", "legend_item").attr("id", function (d) {
      return "legend_item_" + d;
      ...
      s.append("rect").attr("height", "0.6em").attr("width", "0.6em").attr("fill", function (d) {
        return that.chart._color(d);
      }).attr("y", "0.2em").attr("x", "0.0em").attr("font-size", that.font_size);
    }).call(function (s) {
      s.append("text") //.text(function(d,i){ return dataset[i].name; })
      .attr("y", "0.8em").attr("x", "0.8em").attr("font-size", that.font_size);
    }); // update

    item.select("rect").attr("visibility", function (d, i) {
      return dataset[i].visible ? "visible" : "hidden";
    });
    item.select("text").text(function (d, i) {
  ...
  ccd3.Parts.Legend.prototype.toggle_series = function (series_num) {
    var dataset = this.chart.dataset;
    dataset[series_num].visible = !dataset[series_num].visible; // reset zoomed

    this.chart.svg.selectAll(".ccd3_zoom_reset_button").remove();
    this.chart.zoomed = false; //this.chart.render({ dont_reset_domain:true  });
  ...
  ccd3.Parts.Tooltip.prototype.render = function () {
    ...
    var that = this; // initialize format	

    this.xFormat = this.xFormat || this.chart.xAxis.format || this.chart.aAxis.format;
    this.yFormat = this.yFormat || this.chart.yAxis.format || this.chart.rAxis.format; //this.tooltip.div = d3.select("#"+this.div_id)

    this.div = d3.select("body").append("div").attr("id", "tooltip_" + this.chart.div_id).attr("class", "ccd3_tooltip").style("opacity", 0);
    this.svg = this.div;
    ...
    if (this.chart.event_layer.svg && this.axis_scan && (this.chart.direction == "x" || this.chart.direction == "y")) {
      this.chart.event_layer.svg.call(this.add_axis_tooltip_listener, this);

      if (this.show_scan_line) {
        this.chart.event_layer.svg.insert("rect", ".ccd3_zoom") // use insert not to prevent zoom
        .attr("visibility", "none").attr("class", "ccd3_axis_scan_line").attr("fill", "gray").attr("opacity", 0.2);
      }
    }
  };
  ...
  ccd3.Parts.Tooltip.prototype.add_listener = function (e, that) {
    e.on("mouseover.tooltip", function (d) {
      ...
      that.div.html(that.chart.series[series_num].tooltip_html(d) + that.annotation_func(d)).style("display", null) //.transition().duration(300)
      .style("opacity", 0.8);
    });
    e.on("mouseout.tooltip", function () {
      ccd3.get_instance(this).tooltip.div //.transition().duration(300)
      .style("opacity", 0).style("display", "none");
    });
    e.on("mousemove.tooltip", function () {
      ccd3.get_instance(this).tooltip.div.style("display", function () {
        return that.div[0][0].clientWidth > 0 ? null : "none";
      }).style("top", d3.event.pageY + 10 + "px").style("left", function () {
  ...
  ccd3.Parts.Tooltip.prototype.add_axis_tooltip_listener = function (e, that) {
    ...
    e.on("mouseover.axis_tooltip", function (d) {
      that.chart.tooltip.div.style("display", null).style("opacity", 0.8);
      that.chart.event_layer.svg.select(".ccd3_axis_scan_line").attr("visibility", "visible");
      prev_data_x = null;
      prev_data_y = null;
    });
    e.on("mouseout.axis_tooltip", function () {
      ccd3.get_instance(this).tooltip.div.style("opacity", 0).style("display", "none");
    ...
    if (that.chart.direction == "x") {
      e.on("mousemove.axis_tooltip", function () {
        ...
        that.div.style("display", function () {
          return that.div[0][0].clientWidth > 0 ? null : "none";
        }).style("top", d3.event.pageY + 10 + "px").style("left", function () {
          var div_width = that.div[0][0].clientWidth;
          var offset_x = d3.mouse(d3.select("#" + that.chart.div_id)[0][0])[0];
        ...
        prev_data_x = data_x;
        var values = that.chart.dataset_manager.lookup("x", data_x, true); // highlight series
        ...
        that.div.html(html); // show axis scan line

        if (that.show_scan_line) {
          if (that.chart.dataset_manager.has_series_type("bar") || that.chart.dataset_manager.has_series_type("stackedbar")) {
            if (xAxis.scale_type == "ordinal") {
          ...
          var line_x = xAxis.scale(values[0].x) - that.scan_line_width / 2 + (xAxis.scale_type == "ordinal" ? band_width / 2 : 0);
          that.chart.event_layer.svg.select(".ccd3_axis_scan_line").attr("x", line_x).attr("width", that.scan_line_width).attr("y", 0).attr("height", that.chart.inner_height).attr("visibility", "visible");
        }
      });
      ...
      e.on("mousemove.axis_tooltip", function () {
        ...
        prev_data_y = data_y;
        var values = that.chart.dataset_manager.lookup("y", data_y, true); // highlight series
        ...
        if (that.show_scan_line) {
          ...
          var line_y = yAxis.scale(values[0].y) - that.scan_line_width / 2 + (yAxis.scale_type == "ordinal" ? band_width / 2 : 0);
          that.chart.event_layer.svg.select(".ccd3_axis_scan_line").attr("y", line_y).attr("height", that.scan_line_width).attr("x", 0).attr("width", that.chart.inner_width).attr("visibility", "visible");
        }
      });
    }
  };
  ...
  ccd3.Parts.Menu.prototype.render = function () {
    ...
    if (this.svg === undefined) {
      ...
      this.svg_icon.append("rect").attr("class", "ccd3_menu_rect").attr("width", this.icon_width).attr("height", this.icon_height).attr("fill", "white").attr("opacity", 0);
      ...
      this.svg_icon.selectAll(".ccd3_menu_bar").attr("width", 20).attr("height", 2).attr("rx", 2).attr("ry", 2).attr("x", 2).attr("fill", "gray");
  ...
  ccd3.Parts.Menu.prototype.toggle_menu = function () {
    ...
    var menus = this.svg_list.selectAll(".ccd3_menu_item").data(data); // enter

    menus.enter().append("g").attr("class", "ccd3_menu_item").attr("cursor", "pointer").call(function (e) {
      e.append("rect").attr("fill", "white").attr("stroke", "gray").attr("opacity", that.item_opacity).attr("rx", 8).attr("ry", 8);
    }).call(function (e) {
      e.append("text").text(function (d) {
        return d.label;
    ...
    }).attr("transform", closed_xy); // update

    menus.selectAll(".ccd3_menu_item rect").attr("x", function () {
      return that.item_x_margin * -1;
    }).attr("y", function () {
      return this.nextSibling.getBBox().height / -2 + that.item_y_margin * -2;
    }).attr("width", function () {
      return this.nextSibling.getBBox().width + that.item_x_margin * 2 + 30;
    }).attr("height", function () {
      return this.nextSibling.getBBox().height + that.item_y_margin * 2;
    });
    menus.transition().duration(500).attr("transform", opened_xy); // exit

    menus.exit().transition().duration(500).attr("transform", closed_xy).remove();
  };
  /* ------------------------------------------------------------------ */
  ...
  ccd3.Parts.OverlayLoading = function () {
    ccd3.Parts.apply(this, arguments);
  };
  ...
  ccd3.Parts.OverlayLoading.prototype.render = function (onoff) {
    ...
    if (this.svg === undefined) {
      ...
      this.svg.append("rect").attr("class", "ccd3_overlay_rect").attr("fill", "white").attr("opacity", 0.6);
      this.svg.append("text").attr("class", "ccd3_overlay_message").attr("font-size", this.font_size);
    }

    this.svg.select(".ccd3_overlay_rect").attr("width", that.chart.width).attr("height", that.chart.height);
    this.svg.select(".ccd3_overlay_message").text("Now Loading ...").attr("x", function () {
  ...
  ccd3.Parts.Axis.prototype.get_defaults = function () {
    return {
      ...
      domain_margin_type: undefined,
      // switch using domain_margin or min_step
      band_padding: 0.2,
      // also ref from Series.Bar/StackedBar
      format: undefined,
      show_label: true,
      tick_num: undefined,
      tick_padding: 5,
      tick_values: undefined,
      // use if scale_type == "date"
      font_size: 10,
      label_width: 0,
      label_height: 0,
      margin_top: 3,
  ...
  ccd3.Parts.Axis.prototype.reset_range = function (range_max) {
    ...
    if (this.scale_type === "ordinal") {
      ...
      this.scale.range([this.range_min, this.range_max]); // not to show same tick_label twice, force tickValues by Axis.format
  ...
  ccd3.Parts.Axis.prototype.reset_domain = function () {
    ...
    if (this.scale_type === "ordinal") {
      ...
      if (this.domain_min !== undefined && this.domain_min !== null) {
        min = this.domain_min;
      }

      if (this.domain_max !== undefined && this.domain_max !== null) {
        max = this.domain_max;
      }
  ...
  ccd3.Parts.Axis.xAxis = function () {
    ccd3.Parts.Axis.apply(this, arguments);
  };
  ...
  ccd3.Parts.Axis.xAxis.prototype.render = function () {
    ...
    this.axis = d3.svg.axis().scale(this.scale).tickSize(-1 * height).orient("bottom").tickFormat(format);
    ...
    .call(this.axis).selectAll(".tick text").style("text-anchor", "end").attr("class", "ccd3_x_tick_label").attr("font-size", this.font_size).attr("transform", function () {
    ...
    this.svg.selectAll(".ccd3_xAxis path, .ccd3_xAxis .tick line").attr("fill", "none").attr("stroke", "lightgray").attr("stroke-width", 1);
  ...
  ccd3.Parts.Axis.yAxis = function () {
    ccd3.Parts.Axis.apply(this, arguments);
  };
  ...
  ccd3.Parts.Axis.yAxis.prototype.render = function () {
    ...
    this.axis = d3.svg.axis().scale(this.scale).tickSize(-1 * width).tickPadding(this.tick_padding).orient("left").tickFormat(format);
    ...
    this.svg //.transition().duration(500)
    .call(this.axis).selectAll(".tick text").attr("class", "ccd3_y_tick_label").attr("font-size", this.font_size);
    this.svg.selectAll(".tick text").each(function (e) {
      var t = d3.select(this).text();
      d3.select(this).text(ccd3.Util.extract_axis_text(t));
    });
    this.svg.selectAll(".ccd3_yAxis path, .ccd3_yAxis .tick line").attr("fill", "none").attr("stroke", "lightgray").attr("stroke-width", 1);
  ...
  ccd3.Parts.rAxis.prototype.render = function () {
    var that = this;
    this.init_scale(); //this.reset_domain();

    if (!this.svg) {
      this.svg = this.chart.svg.append("g").attr("class", "ccd3_rAxis");
    }

    var ticks = this.svg.selectAll(".ccd3_rAxis_ticks").data(that.scale.ticks(3)); // enter

    ticks.enter().append("g").attr("class", "ccd3_rAxis_ticks").call(function (e) {
      e.append("circle").attr("fill", "none").attr("stroke", "lightgray").attr("stroke-width", 1);
    }).call(function (e) {
      e.append("text").attr("text-anchor", "middle").attr("font-size", that.font_size);
    }); // update

    ticks.call(function (e) {
      e.select("circle").attr("r", function (d) {
        return that.scale(d);
      });
    }).call(function (e) {
      e.select("text").text(function (d) {
        return that.format(d);
      }).attr("y", function (d) {
        return -1.0 * that.scale(d);
      });
    }); // exit
  ...
  ccd3.Parts.aAxis.prototype.render = function () {
    ...
    var ticks = this.svg.selectAll(".ccd3_aAxis_ticks").data(this.scale.domain()); // enter

    ticks.enter().append("g").attr("class", "ccd3_aAxis_ticks").call(function (e) {
      e.append("line").attr("x1", 0).attr("y1", 0).attr("stroke", "lightgray").attr("stroke-width", 1);
    }).call(function (e) {
      e.append("text").attr("text-anchor", "middle").attr("font-size", that.font_size);
    }); // update

    ticks.call(function (e) {
      e.select("line").attr("x2", function (d) {
        return that.radius * Math.sin(that.scale(d));
      }).attr("y2", function (d) {
        return that.radius * Math.cos(that.scale(d)) * -1.0;
      });
    }).call(function (e) {
      e.select("text").text(function (d) {
        return that.format(d);
        ...
        if (x + w / 2 > that.chart.inner_width / 2) {
          // not to overflow
  ...
  ccd3.Parts.EventLayer = function () {
    ccd3.Parts.apply(this, arguments);
  };

  ccd3.Parts.EventLayer.prototype = new ccd3.Parts();

  ccd3.Parts.EventLayer.prototype.get_defaults = function () {
    return {};
  };

  ccd3.Parts.EventLayer.prototype.render = function () {
    if (this.svg === undefined) {
      this.svg = this.chart.series_container.svg.append("g").attr("class", "ccd3_event_layer");
      this.svg.append("rect").attr("class", "background").attr("opacity", 0).attr("x", 0).attr("y", 0);
    }

    this.svg.select("rect.background").attr("width", this.chart.inner_width).attr("height", this.chart.inner_height);
  };
  /* ------------------------------------------------------------------ */
  ...
  ccd3.Parts.Zoom.prototype.render = function () {
    ...
    var that = this;
    this.brush = d3.svg.brush().x(this.chart.xAxis.scale).y(this.chart.yAxis.scale).on("brushend", function () {
      that.zoom();
    });
    this.svg = this.chart.event_layer.svg.append("g").attr("class", "ccd3_zoom").attr("cursor", "crosshair").attr("fill", "gray").attr("opacity", 0.3).call(this.brush);
  };
  ...
  ccd3.Parts.Zoom.prototype.zoom = function () {
    ...
    this.button_svg = c.series_container.svg.append("g").attr("class", "ccd3_zoom_reset_button").attr("cursor", "pointer").call(function (e) {
      e.append("rect").attr("height", "1.6em").attr("font-size", that.font_size).attr("fill", "lightgray").attr("opacity", 0.7).attr("stroke", "none").attr("rx", 5).attr("ry", 5).attr("x", -5);
    }).call(function (e) {
      e.append("text").text("clear zoom").attr("fill", "#333333").attr("text-anchor", "left").attr("font-size", that.font_size).attr("y", "1.2em");
    }).on("click", function () {
      that.reset_zoom();
    });
    this.button_svg.call(function (e) {
      e.select("rect").attr("width", function () {
        return this.nextSibling.getBBox().width + 10;
      });
    });
    this.rearrange_reset_button(); // callback
  ...
  ccd3.Parts.Zoom.prototype.reset_zoom = function () {
    ...
    c.zoomed = false;
    c.render();
    c.zoom.remove_reset_button(); // callback

    if (this.onzoom_reset) {
      this.onzoom_reset({
        x_min: c.xAxis.reset_domain_min,
        x_max: c.xAxis.reset_domain_max,
        y_min: c.yAxis.reset_domain_min,
        y_max: c.yAxis.reset_domain_max
      });
    }
  };
  ...
  ccd3.Parts.Zoom.prototype.rearrange_reset_button = function () {
    if (this.button_svg) {
      var that = this;
      this.button_svg.attr("transform", function () {
  ...
  ccd3.Parts.Series.type_info = function () {
    return {
      heatmap: {
        obj: ccd3.Parts.Series.Heatmap,
        zindex: 1,
        is_single_series: true
      },
      bar: {
        obj: ccd3.Parts.Series.Bar,
        zindex: 2
      },
      stackedbar: {
        obj: ccd3.Parts.Series.StackedBar,
        zindex: 3
      },
      bubble: {
        obj: ccd3.Parts.Series.Bubble,
        zindex: 4
      },
      line: {
        obj: ccd3.Parts.Series.Line,
        zindex: 5
      },
      scatter: {
        obj: ccd3.Parts.Series.Scatter,
        zindex: 6
      },
      pie: {
        obj: ccd3.Parts.Series.Pie,
        zindex: 0,
        is_single_series: true
      },
      radar: {
        obj: ccd3.Parts.Series.Radar,
        zindex: 0
      }
    };
  };
  ...
  ccd3.Parts.Series.prototype.init_svg = function (series) {
    ...
    if (!this.svg) {
      ...
      this.svg = this.chart.series_container.svg.append("g").attr("id", "series_" + series.series_num).attr("class", "series").attr("series_num", series.series_num).attr("series_name", series.name).attr("class", this.class_name).attr("series_type", series.series_type).datum(info[series.series_type].zindex * 1000 + series.series_num) // magic number 1000
      ;
    }
  };
  ...
  ccd3.Parts.Series.prototype.exit = function (s) {
    s.exit().transition().duration(500).ease("linear").style("opacity", 0).remove();
  };
  ...
  ccd3.Parts.Series.prototype.text_visibility = function () {
    if (this.show_values || this.chart.show_values === "always" || this.chart.zoomed && this.chart.show_values === "onzoom") {
      return true;
    } else {
      return false;
    }
  ...
  ccd3.Parts.Series.Scatter = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Scatter.prototype.render = function () {
    ...
    this.color = this.chart._color(i); // data join

    var circles = this.svg.selectAll(".ccd3_circle_g").data(that.get_data()); // enter

    circles.enter().append("g").attr("class", "ccd3_circle_g").style("opacity", 0).call(function (e) {
      e.append("circle").attr("fill", that.color).attr("r", that.point_radius);
    }).call(function (e) {
      e.append("text").attr("text-anchor", "middle").attr("y", "-0.5em").text(function (d) {
    ...
    }).call(that.chart.tooltip.add_listener, that.chart.tooltip).call(function (e) {
    ...
    circles.transition().duration(500).attr("transform", function (d) {
      ...
      return "translate(" + x + "," + y + ")";
    }).style("opacity", 1).call(function (e) {
      e.select("text").style("display", function () {
        return text_visibility ? null : "none";
      }).attr("font-size", that.font_size);
    }); // exit
  ...
  ccd3.Parts.Series.Scatter.prototype.highlight_series = function (onoff) {
    if (onoff) {
      this.svg.selectAll(".ccd3_circle_g circle").attr("r", this.point_radius_highlight);
    } else {
      this.svg.selectAll(".ccd3_circle_g circle").attr("r", this.point_radius);
    }
  };

  ccd3.Parts.Series.Scatter.prototype.highlight = function (data) {
    var that = this;
    this.svg.selectAll(".ccd3_circle_g circle").attr("r", function (d) {
      if ("" + d.x == "" + data.x && "" + d.y == "" + data.y) {
        // to compare Date object converting to string
  ...
  ccd3.Parts.Series.Bar = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Bar.prototype.render = function (bar_cnt, bar_pos) {
    ...
    var bars = this.svg.selectAll(".ccd3_bar_g").data(that.get_data()); // enter

    bars.enter().append("g").attr("class", "ccd3_bar_g").attr("transform", "rotate(0)").style("opacity", 0).call(function (e) {
      e.append("rect").attr("fill", that.color);
    }).call(function (e) {
      e.append("text").attr("transform", "rotate(0)").text(function (d) {
        if (that.chart.show_values_format) {
          return that.chart.show_values_format(d);
        } else if (d.label) {
          return d.label;
    ...
    bars.transition().duration(500).attr("transform", xy_func).style("opacity", that.opacity).call(function (e) {
      e.select("rect").attr("width", width_func).attr("height", height_func);
    }).call(function (e) {
      e.select("text").attr("visibility", function () {
        return text_visibility ? "visible" : "hidden";
      }).attr("transform", text_xy_func).attr("font-size", that.font_size);
    }); // exit
  ...
  ccd3.Parts.Series.Bar.prototype.highlight_series = function (onoff) {
    if (onoff) {
      this.svg.selectAll(".ccd3_bar_g").style("opacity", 1);
    } else {
      this.svg.selectAll(".ccd3_bar_g").style("opacity", this.opacity);
    }
  };

  ccd3.Parts.Series.Bar.prototype.highlight = function (data) {
    var that = this;
    this.svg.selectAll(".ccd3_bar_g").style("opacity", function (d) {
      if ("" + d.x == "" + data.x && "" + d.y == "" + data.y) {
        // to compare Date object converting to string
        return 1;
      } else {
  ...
  ccd3.Parts.Series.StackedBar = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.StackedBar.prototype.render = function () {
    ...
    bars.enter().append("g").attr("class", "ccd3_bar_g").style("opacity", 0).call(function (e) {
      e.append("rect").attr("fill", that.color);
    }).attr("transform", "rotate(0)").call(function (e) {
      e.append("text").text(function (d) {
        if (that.chart.show_values_format) {
          return that.chart.show_values_format(d);
        } else if (d.label) {
    ...
    bars.transition().duration(500) //.delay(function(d, i) { return i * 10; })
    .attr("transform", xy_func).style("opacity", that.opacity).call(function (e) {
      e.select("rect").attr("width", width_func).attr("height", height_func);
    }).call(function (e) {
      e.select("text").attr("visibility", function () {
        return text_visibility ? "visible" : "hidden";
  ...
  ccd3.Parts.Series.Line = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Line.prototype.render = function () {
    ...
    var path_def = d3.svg.line().x(calc_x).y(calc_y).interpolate("linear");
    var path = this.svg.selectAll("path").data(data); // enter

    path.enter().append("path").attr("class", "series_path").attr("stroke", that.color).attr("stroke-width", that.line_width).attr("fill", "none").style("opacity", 0); // update

    path.transition().duration(500).attr("d", path_def(that.get_data())).style("opacity", that.opacity); // exit
    ...
    circles.enter().append("g").attr("class", "ccd3_circle_g").attr("transform", "rotate(0)").style("opacity", 0).call(function (e) {
      e.append("circle").attr("fill", that.color).attr("r", that.point_radius);
    }).call(function (e) {
      e.append("text").attr("text-anchor", "middle").attr("y", "-0.5em").text(function (d) {
        if (that.chart.show_values_format) {
      ...
      d3.select(this).select("circle").attr("r", that.point_radius_highlight);
      d3.select(this.parentNode).select(".series_path").attr("stroke-width", that.line_width_highlight);
    }).on("mouseout", function () {
      d3.select(this).select("circle").attr("r", that.point_radius);
      d3.select(this.parentNode).select(".series_path").attr("stroke-width", that.line_width);
    }).call(that.chart.tooltip.add_listener, that.chart.tooltip).call(function (e) {
      if (that.onclick_customize_func) {
        e.on("click.customize_func", that.onclick_customize_func);
        e.attr("cursor", "pointer");
    ...
    circles.transition().duration(500).attr("transform", "rotate(0)").attr("transform", function (d) {
  ...
  ccd3.Parts.Series.Line.prototype.highlight_series = function (onoff) {
    if (onoff) {
      this.svg.selectAll(".ccd3_circle_g circle").attr("r", this.point_radius_highlight);
      this.svg.selectAll(".series_path").attr("stroke-width", this.line_width_highlight);
    } else {
      this.svg.selectAll(".ccd3_circle_g circle").attr("r", this.point_radius);
      this.svg.selectAll(".series_path").attr("stroke-width", this.line_width);
    }
  };
  ...
  ccd3.Parts.Series.Bubble = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Bubble.prototype.render = function () {
    ...
    if (this.draw_line) {
      ...
      // data join
      var data = this.series.visible ? [0] : [];
      var path_def = d3.svg.line().x(function (d) {
        return xScale(d.x);
      }).y(function (d) {
        return yScale(d.y);
      }).interpolate("linear");
    ...
    var circles = this.svg.selectAll(".ccd3_bubble_g").data(that.get_data()); // enter

    circles.enter().append("g").attr("class", "ccd3_bubble_g").style("opacity", 0).call(function (e) {
      e.append("circle").attr("fill", that.color).attr("r", 0);
    }).call(function (e) {
      e.append("text").attr("text-anchor", "middle").attr("y", "0.4em").text(function (d) {
        if (that.chart.show_values_format) {
          return that.chart.show_values_format(d);
        } else if (d.label) {
          return d.label;
    ...
    circles.transition().duration(500).attr("transform", function (d) {
      ...
      return "translate(" + x + "," + y + ")";
    }).style("opacity", that.opacity).call(function (e) {
      e.select("circle").attr("r", function (d) {
        var z = zScale(d.z);
      ...
      e.select("text").attr("visibility", function () {
        return text_visibility ? "visible" : "hidden";
      }).attr("font-size", that.font_size).attr("text-anchor", function () {
        var bb = this.getBBox();
  ...
  ccd3.Parts.Series.Bubble.prototype.highlight_series = function (onoff) {
    if (onoff) {
      this.svg.selectAll(".ccd3_bubble_g").style("opacity", 1);
    } else {
      this.svg.selectAll(".ccd3_bubble_g").style("opacity", this.opacity);
    }
  };

  ccd3.Parts.Series.Bubble.prototype.highlight = function (data) {
    var that = this;
    this.svg.selectAll(".ccd3_bubble_g").style("opacity", function (d) {
      if ("" + d.x == "" + data.x && "" + d.y == "" + data.y) {
        // to compare Date object converting to string
        return 1.0;
      } else {
  ...
  ccd3.Parts.Series.Heatmap = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Heatmap.prototype.render = function () {
    ...
    width += 1;
    height += 1; // cScale = color Scale

    var zMax = this.chart.dataset_manager.get_max(function (d) {
      return d.z;
    });
    var zMin = this.chart.dataset_manager.get_min(function (d) {
      return d.z;
    ...
    var rects = this.svg.selectAll(".ccd3_rect_g").data(that.get_data()); // enter

    rects.enter().append("g").attr("transform", "rotate(0)").attr("class", "ccd3_rect_g").style("opacity", 0).call(function (e) {
      e.append("rect");
    }).call(function (e) {
      e.append("text").attr("transform", "rotate(0)").attr("font-size", that.font_size).attr("text-anchor", "middle");
    }).call(that.chart.tooltip.add_listener, that.chart.tooltip).call(function (e) {
      if (that.onclick_customize_func) {
        e.on("click.customize_func", that.onclick_customize_func);
        e.attr("cursor", "pointer");
    ...
    rects.transition().duration(500).style("opacity", that.opacity).attr("transform", xy_func).call(function (e) {
      e.select("rect").attr("width", width).attr("height", height).attr("fill", function (d) {
        return that.cScale(d.z);
      });
    }).call(function (e) {
      e.select("text").attr("visibility", function () {
        return text_visibility ? "visible" : "hidden";
      }).attr("transform", text_xy_func).text(function (d) {
        if (that.chart.show_values_format) {
          return that.chart.show_values_format(d);
        } else if (d.label) {
          return d.label;
  ...
  ccd3.Parts.Series.Pie = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Pie.prototype.render = function () {
    ...
    var arcs = this.svg.selectAll(".ccd3_arc_g").data(pie_data); // enter

    arcs.enter().append("g").attr("class", "ccd3_arc_g").style("opacity", this.opacity).call(that.chart.tooltip.add_listener, that.chart.tooltip).call(function (e) {
      if (that.onclick_customize_func) {
        e.on("click.customize_func", that.onclick_customize_func);
        e.attr("cursor", "pointer");
      }
    }).on("mouseover", function () {
      d3.select(this).style("opacity", 1).select("path").attr("d", function (d, i) {
        arc.outerRadius(that.radius);
        return arc(d);
      });
    }).on("mouseout", function () {
      d3.select(this).style("opacity", that.opacity).select("path").attr("d", function (d, i) {
        arc.outerRadius(that.radius * that.radius_nofocus);
        return arc(d);
      });
    }).call(function (e) {
      e.append("path").style("fill", function (d, i) {
        return that.chart._color(i);
      });
    }).call(function (e) {
      e.append("text").attr("dy", ".35em").style("text-anchor", "middle").attr("transform", function (d) {
        return "translate(" + arc.centroid(d) + ")";
      }).call(function (e) {
        e.append("tspan").attr("class", "label_x").attr("y", "-1em").text(function (d) {
          return d.data.x;
        });
      }).call(function (e) {
        e.append("tspan").attr("class", "label_p").attr("dy", "1em").attr("x", 0).text(function (d) {
          return that.pFormat(that.calc_percent(d));
        });
      }).call(function (e) {
        e.append("tspan").attr("class", "label_y").attr("dy", "1em").attr("x", 0).text(function (d) {
          return that.yFormat(d.data.y);
        });
      });
    }); // update

    arcs.transition().duration(500).call(function (e) {
      e.select("path").attr("d", arc);
    }).call(function (e) {
      e.select("text").attr("transform", function (d) {
        return "translate(" + arc.centroid(d) + ")";
      }).call(function (e) {
        e.select("tspan.label_x").text(function (d) {
          return d.data.x;
        });
      }).call(function (e) {
        e.select("tspan.label_p").text(function (d) {
          return that.pFormat(that.calc_percent(d));
        });
      }).call(function (e) {
        e.select("tspan.label_y").text(function (d) {
          return that.yFormat(d.data.y);
        });
  ...
  ccd3.Parts.Series.Radar = function () {
    ccd3.Parts.Series.apply(this, arguments);
  };
  ...
  ccd3.Parts.Series.Radar.prototype.render = function () {
    ...
    var poly = this.svg.selectAll(".ccd3_poly").data(poly_data); // enter

    poly.enter().append("polygon").attr("class", "ccd3_poly").attr("stroke", that.color).attr("stroke-width", 2).attr("fill", that.color).attr("fill-opacity", 0.1).on("mouseover", function () {
      d3.select(this).style("fill-opacity", 0.5);
    }).on("mouseout", function () {
      d3.select(this).style("fill-opacity", 0.1);
    }); // update

    poly.transition().duration(500).attr("points", function (d) {
      var str = "",
          xy;
    ...
    var points = this.svg.selectAll(".ccd3_points_g").data(data); // enter

    points.enter().append("g").call(that.chart.tooltip.add_listener, that.chart.tooltip).call(function (e) {
      if (that.onclick_customize_func) {
        e.on("click.customize_func", that.onclick_customize_func);
        e.attr("cursor", "pointer");
      }
    }).attr("class", "ccd3_points_g").call(function (e) {
      e.append("circle").attr("fill", that.color).attr("r", that.point_radius);
    }).on("mouseover", function () {
      d3.select(this).select("circle").attr("r", that.point_radius_highlight);
    }).on("mouseout", function () {
    ...
    points.transition().duration(500).attr("transform", function (d) {
      var xy = trans_xy(d);
  ...
  ccd3.Parts.Series.Radar.prototype.highlight_series = function (onoff) {
    if (onoff) {
      this.svg.selectAll(".ccd3_poly").style("fill-opacity", 0.5);
    } else {
      this.svg.selectAll(".ccd3_poly").style("fill-opacity", 0.1);
    }
  };
  ...
  ccd3.DatasetManager.prototype.setup = function (dataset) {
    var i, len;
    var type_info = ccd3.Parts.Series.type_info(); // deep copy dataset

    this.dataset = ccd3.Util.copy(dataset);
    this.dataset_original = ccd3.Util.copy(dataset);
    dataset = this.dataset; // set default properties
  ...
  ccd3.DatasetManager.prototype.update_stack_values = function (direction, stack_type) {
    ...
    if (dataset_stackedbar.length > 0) {
      ...
      if (direction === "x") {
        stack = d3.layout.stack().offset(stack_type).values(function (d) {
          return d.values;
        });
      ...
      for (i = 0; i < dataset.length; i++) {
        for (j = 0; j < dataset[i].values.length; j++) {
          ...
          if (d.x !== undefined && d.x !== null) {
            row.push(xFormat(ccd3.Util.extract_axis_text(d.x)));
          } else {
            row.push("");
          }

          if (d.y !== undefined && d.y !== null) {
            row.push(yFormat(ccd3.Util.extract_axis_text(d.y)));
          } else {
            row.push("");
          }

          if (d.z !== undefined && d.z !== null) {
            row.push(zFormat(ccd3.Util.extract_axis_text(d.z)));
          } else {
            row.push("");
          }
